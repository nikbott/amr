...
inline T get(std::istream& is) {
    ...
    if (!is) throw std::runtime_error("mesh_io: unexpected end of file");
    return v;
}

inline uint64_t pad_to_8(uint64_t n) { return (8u - (n % 8u)) % 8u; }
...
inline void write(const std::string& path, const MeshData& m) {
    ...
    if (!os) throw std::runtime_error("mesh_io::write: cannot open " + path);
    ...
    for (uint32_t k = 0; k < m.dim; ++k) detail::put<double>(os, m.origin[k]);
    for (uint32_t k = 0; k < m.dim; ++k) detail::put<double>(os, m.size[k]);

    if (n) os.write(reinterpret_cast<const char*>(m.codes.data()),
                    static_cast<std::streamsize>(n * sizeof(uint64_t)));
    if (n) os.write(reinterpret_cast<const char*>(m.levels.data()),
                    static_cast<std::streamsize>(n));
    const uint64_t pad = detail::pad_to_8(n);
    for (uint64_t i = 0; i < pad; ++i) os.put('\0');
    ...
    for (const auto& f : m.fields) {
        ...
        if (f.f64) {
            ...
            for (uint64_t i = 0; i < n; ++i) detail::put<float>(os, static_cast<float>(f.values[i]));
        }
    }
    if (!os) throw std::runtime_error("mesh_io::write: write failed for " + path);
}
...
inline MeshData read(const std::string& path) {
    ...
    if (!is) throw std::runtime_error("mesh_io::read: cannot open " + path);
    ...
    if (version != 1) throw std::runtime_error("mesh_io::read: unsupported version");

    MeshData m;
    m.dim = detail::get<uint32_t>(is);
    if (m.dim != 2 && m.dim != 3) throw std::runtime_error("mesh_io::read: bad dim");
    m.max_level = detail::get<uint32_t>(is);
    const uint64_t n = detail::get<uint64_t>(is);
    for (uint32_t k = 0; k < m.dim; ++k) m.origin[k] = detail::get<double>(is);
    for (uint32_t k = 0; k < m.dim; ++k) m.size[k] = detail::get<double>(is);

    m.codes.resize(n);
    m.levels.resize(n);
    if (n) is.read(reinterpret_cast<char*>(m.codes.data()),
                   static_cast<std::streamsize>(n * sizeof(uint64_t)));
    if (n) is.read(reinterpret_cast<char*>(m.levels.data()), static_cast<std::streamsize>(n));
    if (!is) throw std::runtime_error("mesh_io::read: truncated codes/levels");
    ...
    for (uint32_t fi = 0; fi < n_fields; ++fi) {
        ...
        if (name_len) is.read(f.name.data(), static_cast<std::streamsize>(name_len));
        const uint32_t dtype = detail::get<uint32_t>(is);
        f.f64 = (dtype == 1);
        f.values.resize(n);
        if (f.f64) {
            if (n) is.read(reinterpret_cast<char*>(f.values.data()),
                           static_cast<std::streamsize>(n * sizeof(double)));
        } else {
            for (uint64_t i = 0; i < n; ++i) f.values[i] = static_cast<double>(detail::get<float>(is));
        }
        if (!is) throw std::runtime_error("mesh_io::read: truncated field '" + f.name + "'");
