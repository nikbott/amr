...
int count_balance_violations(const DistributedTree<DIM>& tree) {
    ...
    for (const auto& node : tree) {
        ...
        for (const auto& dir : dirs) {
            ...
            if constexpr (DIM == 2) target_code = morton::encode_2d(target_pt[0], target_pt[1]);
            else target_code = morton::encode_3d(target_pt[0], target_pt[1], target_pt[2]);
            ...
            // Check Local
            auto it = std::lower_bound(tree.begin(), tree.end(), search_key, 
                [](const Node& a, const Node& b){ return a.code < b.code; });
            ...
            if (neighbor_level == -100) {
                auto git = std::lower_bound(ghosts.begin(), ghosts.end(), search_key, 
                     [](const Node& a, const Node& b){ return a.code < b.code; });
...
TEMPLATE_TEST_CASE("Linear Tree Invariants (Burstedde §2.2)", "[tree]", DistributedTree<2>, DistributedTree<3>) {
    ...
    SECTION("Neighbor Query Integrity") {
        ...
        MortonCode code;
        if constexpr (DIM == 2) code = morton::encode_2d(coords[0], coords[1]);
        else code = morton::encode_3d(coords[0], coords[1], coords[2]);
...
TEMPLATE_TEST_CASE("Adaptivity & Coarsening (Burstedde §3.2)", "[amr][coarsen]", DistributedTree<2>, DistributedTree<3>) {
    constexpr int DIM = (std::is_same<TestType, DistributedTree<2>>::value) ? 2 : 3;
...
TEMPLATE_TEST_CASE("2:1 Balance & Ripple Algorithm (Holke §3.3)", "[balance]", DistributedTree<2>, DistributedTree<3>) {
    constexpr int DIM = (std::is_same<TestType, DistributedTree<2>>::value) ? 2 : 3;
