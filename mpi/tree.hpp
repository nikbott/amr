...
struct Node {
    ...
    bool operator<(const Node& other) const { 
        if (code.value != other.code.value) return code.value < other.code.value;
        return level < other.level;
    }
    bool operator==(const Node& other) const { return code.value == other.code.value && level == other.level; }
};
...
public:
    ...
    [[nodiscard]] size_t global_size() const { return mpi::all_reduce_sum<uint64_t>(leaf_codes.size()); }
    ...
    public:
        ...
        using pointer = void; using reference = Node;
        ConstIterator(const DistributedTree* t, size_t i) : tree(t), index(i) {}
        reference operator*() const { return (*tree)[index]; }
        struct ArrowProxy { Node value; const Node* operator->() const { return &value; } };
    ...
    MortonCode get_neighbor_code(MortonCode code, int level, const std::array<int, DIM>& dir) const {
        ...
        auto add_dim = [&](uint64_t current, uint64_t dim_mask, int d) -> uint64_t {
            if (current == std::numeric_limits<uint64_t>::max()) return current;
            if (d == 0) return current;
            uint64_t shift_coord = max_level - level;
            uint64_t one_dilated = (DIM==3) ? (1ULL << (shift_coord * 3)) : (1ULL << (shift_coord * 2));
            if (dim_mask == mask_y) one_dilated <<= 1;
            if (dim_mask == mask_z) one_dilated <<= 2;
            if (d > 0) {
                 if ((current | ~dim_mask) == std::numeric_limits<uint64_t>::max()) return std::numeric_limits<uint64_t>::max();
                 uint64_t sum = (current | ~dim_mask) + one_dilated;
                 return (sum & dim_mask) | (current & ~dim_mask);
            } else {
                 if ((current & dim_mask) < one_dilated) return std::numeric_limits<uint64_t>::max();
        ...
        if constexpr (DIM == 3) next = add_dim(next, mask_z, dir[2]);
        ...
        while(true) {
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                for (const auto& dir : dirs) {
                    ...
                    if (n_code.value == std::numeric_limits<uint64_t>::max()) continue;
                    uint64_t search_start = n_code.value;
                    uint64_t search_end = n_code.value + my_size;
                    int max_n_level = -1;
                    auto it_local = std::lower_bound(leaf_codes.begin(), leaf_codes.end(), search_start);
                    ...
                    while (it_local != leaf_codes.end() && *it_local < search_end) {
                        max_n_level = std::max(max_n_level, (int)leaf_levels[std::distance(leaf_codes.begin(), it_local)]);
                        ++it_local;
                    }
            ...
            if (!mpi::all_reduce_or(changed)) break;
    ...
    void verify_global() const {
        ...
        for (size_t i=0; i<leaf_codes.size(); ++i) local_vol += std::pow(1.0 / (1ULL << leaf_levels[i]), DIM);
        double total_vol = mpi::all_reduce_sum(local_vol);
        if (std::abs(total_vol - 1.0) > 1e-9) throw std::runtime_error("Partition of Unity Violation");
    }
    ...
    void repartition() {
        ...
        auto get_rank_range = [&](int r) -> std::pair<size_t, size_t> {
            ...
            size_t end   = start + avg + (r_u < rem ? 1 : 0); // Fixed: Comparison is now size_t vs size_t
        ...
        auto exchange_vec = [&](auto& sends, auto& recvs) {
            ...
            for(int i=0; i<mpi_size; ++i) sc[i] = static_cast<int>(sends[i].size() * sizeof(typename std::remove_reference_t<decltype(sends[0])>::value_type));
            
            MPI_Alltoall(sc.data(), 1, MPI_INT, rc.data(), 1, MPI_INT, MPI_COMM_WORLD);
            for(int i=0; i<mpi_size; ++i) { sdisp[i+1] = sdisp[i] + sc[i]; rdisp[i+1] = rdisp[i] + rc[i]; }
            
            std::vector<uint8_t> sflat(sdisp.back()), rflat(rdisp.back());
            for(int i=0; i<mpi_size; ++i) memcpy(sflat.data()+sdisp[i], sends[i].data(), sc[i]);
            
            MPI_Alltoallv(sflat.data(), sc.data(), sdisp.data(), MPI_BYTE,
                          rflat.data(), rc.data(), rdisp.data(), MPI_BYTE, MPI_COMM_WORLD);
            
            for(int i=0; i<mpi_size; ++i) {
                recvs[i].resize(rc[i] / sizeof(typename std::remove_reference_t<decltype(recvs[0])>::value_type));
...
private:
    void update_partition_map() {
        uint64_t first = leaf_codes.empty() ? std::numeric_limits<uint64_t>::max() : leaf_codes.front();
    ...
    int find_owner_of_point(uint64_t code) const {
        ...
        if (rank < 0) return 0;
        if (rank >= mpi_size) return mpi_size - 1;
        return rank;
    }
    ...
    void fetch_ghosts() {
        ...
        uint64_t my_end = leaf_codes.empty() ? 0 : leaf_codes.back() + (1ULL << (DIM*(max_level-leaf_levels.back())));
        #pragma omp parallel 
        {
            ...
            for (size_t i = 0; i < leaf_codes.size(); ++i) {
                ...
                for(auto& d : dirs) {
                    ...
                    if (n.value == std::numeric_limits<uint64_t>::max()) continue;
                    uint64_t n_end = n.value + (1ULL << (DIM*(max_level-lvl)));
                    if (n.value < my_start || n_end > my_end) local_n.push_back({n, lvl});
                }
            }
            #pragma omp critical
        ...
        for(const auto& node : needed) {
            ...
            for(int r = 0; r < mpi_size; ++r) {
                ...
                uint64_t r_end = (r == mpi_size - 1) ? std::numeric_limits<uint64_t>::max() : partition_starts[r+1];
                if (std::max(req_start, r_start) < std::min(req_end, r_end)) send_bufs[r].push_back(node);
            }
        }
        ...
        for(int r = 0; r < mpi_size; ++r) {
            for(const auto& req : recv_bufs[r]) {
                ...
                if (it != leaf_codes.begin()) {
                    ...
                    if (p_c + p_sz > req_s) reply_send[r].push_back({MortonCode{p_c}, (int)leaf_levels[p_idx]});
                }
        ...
        for(const auto& vec : reply_recv) ghost_nodes.insert(ghost_nodes.end(), vec.begin(), vec.end());
        std::sort(ghost_nodes.begin(), ghost_nodes.end());
        ghost_nodes.erase(std::unique(ghost_nodes.begin(), ghost_nodes.end()), ghost_nodes.end());
    }

    template<typename T>
    void exchange_data(const std::vector<std::vector<T>>& sends, std::vector<std::vector<T>>& recvs) {
        std::vector<int> sc(mpi_size), rc(mpi_size), sdisp(mpi_size+1), rdisp(mpi_size+1);
        for(int i=0; i<mpi_size; ++i) sc[i] = sends[i].size() * sizeof(T);
        MPI_Alltoall(sc.data(), 1, MPI_INT, rc.data(), 1, MPI_INT, MPI_COMM_WORLD);
        for(int i=0; i<mpi_size; ++i) { sdisp[i+1] = sdisp[i] + sc[i]; rdisp[i+1] = rdisp[i] + rc[i]; }
        std::vector<uint8_t> sflat(sdisp.back()), rflat(rdisp.back());
        for(int i=0; i<mpi_size; ++i) memcpy(sflat.data()+sdisp[i], sends[i].data(), sc[i]);
