...
#define CHECK(x) if(!(x)) { std::cerr << "FAIL: " << #x << " at line " << __LINE__ << "\n"; exit(1); }
#define REQUIRE(x) CHECK(x)
#define CHECK_NOTHROW(expr) \
    try { \
        expr; \
    } catch (const std::exception& e) { \
        std::cerr << "FAIL: Unexpected exception '" << e.what() << "' for " << #expr << " at line " << __LINE__ << "\n"; \
        exit(1); \
    } catch (...) { \
...
// ==================================================================================
template<int DIM>
int count_balance_violations(const std::vector<uint64_t>& codes, const std::vector<uint8_t>& levels, int max_lvl) {
    int violations = 0;
    int n = codes.size();
    ...
    for (int i=0; i<n; ++i) {
        ...
        if constexpr (DIM == 2) morton::decode_2d(MortonCode{code}, c[0], c[1]);
        else morton::decode_3d(MortonCode{code}, c[0], c[1], c[2]);
        ...
        for (int d=0; d<num_dirs; ++d) {
            ...
            for(int k=0; k<3; ++k) t_coord[k].value = static_cast<uint32_t>(target_pos[k]);

            if constexpr (DIM == 2) target_mc = morton::encode_2d(t_coord[0], t_coord[1]);
            else target_mc = morton::encode_3d(t_coord[0], t_coord[1], t_coord[2]);
...
struct CloudOracle {
    int fine_limit;
    HOST_DEVICE bool operator()(MortonCode mc, int lvl) const {
        if (lvl >= fine_limit) return false;
...
void run_test_invariants() {
    ...
    MortonCode code;
    if constexpr (DIM == 2) code = morton::encode_2d(center, center);
    else code = morton::encode_3d(center, center, center);
    ...
    Coordinate nx, ny, nz;
    if constexpr (DIM == 2) morton::decode_2d(MortonCode{n_code_val}, nx, ny);
    else morton::decode_3d(MortonCode{n_code_val}, nx, ny, nz);
...
void run_test_random_cloud() {
    ...
    for(int i=0; i<8; ++i) tree.refine(c_oracle);
